...
public enum SaveHandler {
    ...
    SaveHandler() {
        ...
        WorldPersistenceHooks.addHook(new WorldPersistenceHooks.WorldPersistenceHook() {
            ...
            @Override
            public CompoundTag getDataForWriting(net.minecraft.world.storage.SaveHandler handler, WorldInfo info) {
                return SaveHandler.this.save(handler, info);
            }

            @Override
            public void readData(net.minecraft.world.storage.SaveHandler handler, WorldInfo info, CompoundTag tag) {
    ...
    private void load(net.minecraft.world.storage.SaveHandler save, WorldInfo worldInfo, CompoundTag base) {
    ...
    private CompoundTag save(net.minecraft.world.storage.SaveHandler save, WorldInfo worldInfo) {
        ...
        for (ModContainer mc : saveHandlers.keySet()) {
            tag = new CompoundTag();
            for (IExternalSaveHandler saveHandler : saveHandlers.get(mc)) {
                CompoundTag n = saveHandler.save(save, worldInfo);
                if (n != null) {
