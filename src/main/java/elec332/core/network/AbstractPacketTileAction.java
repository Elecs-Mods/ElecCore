...
public abstract class AbstractPacketTileAction extends AbstractPacket {
    ...
    public IMessage onMessage(AbstractPacket message, MessageContext ctx) {
        ...
        if (ctx.side == Side.CLIENT) {
            processPacket(WorldHelper.getTileAt(Minecraft.getMinecraft().theWorld, loc), i, data, ctx);
            return null;
        }
        processPacket(WorldHelper.getTileAt(ctx.getServerHandler().playerEntity.getEntityWorld(), loc), i, data, ctx);
        return null;
    }
